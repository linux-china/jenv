...
final JENV_VERSION = '@JENV_VERSION@'
...
rm.get("/res") { req ->
	...
	log purpose, 'jenv', JENV_VERSION, req
...
rm.get("/broadcast/:version") { req ->
	...
	vertx.eventBus.send("mongo-persistor", cmd2){ msg ->
		...
		if(JENV_VERSION == version){
			gtplFile = new File('build/templates/broadcast.gtpl')
			binding = [gvmVersion:JENV_VERSION, vertxVersion:VERTX_VERSION, broadcasts:broadcasts]
		} else {
			gtplFile = new File('build/templates/upgrade.gtpl')
			binding = [version:version, gvmVersion:JENV_VERSION]
		}
